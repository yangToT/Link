...
**客户端 0.2.0-alpha.12 / 服务端 0.2.0-alpha.10，测试版本。** 包含 SSE 状态推送、卸载入口和“功能与组件”页面；局域网接入按需安装，各设备独立启用。Linux 服务端和一台真实 Windows 客户端已连接，具体验证范围见 [验证记录](docs/validation.md)。两台 Windows 跨网互通、系统重启后恢复和实际应用调用尚未验收。
