...
internal static class Common {
 internal const string Version = "0.1.0-alpha.1";
 ...
 internal static string Run(string exe,string args,int timeout=15000){
  var output=new StringBuilder();using(var p=new Process()){p.StartInfo=new ProcessStartInfo(exe,args){UseShellExecute=false,CreateNoWindow=true,RedirectStandardOutput=true,RedirectStandardError=true};
   ...
   if(!p.WaitForExit(timeout)){p.Kill();throw new InvalidOperationException("操作超时");}p.WaitForExit();if(p.ExitCode!=0)throw new InvalidOperationException("系统操作失败，退出码 "+p.ExitCode);return output.ToString();}
 }
