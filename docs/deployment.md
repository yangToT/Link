...
客户端提供 `Uninstall.exe` 和 `uninstall.ps1`，新安装会登记到 Windows 已安装应用。Linux 和 Windows 服务端也提供对应卸载入口；使用方式、清理范围和验证边界见 [卸载说明](uninstall.md)。

## 发布应用

先启动应用，在管理网页选择检测到的监听服务。应用应监听 `0.0.0.0` 或专用网络地址。转发的“可用”表示 TCP 可达，不表示业务健康。

对于需要公布地址的应用，将映射详情中的入口地址保存到应用自身的对外地址配置。应用继续负责注册和心跳；具体配置方式由框架决定，不依赖某个 IDE。入口地址变化后需更新配置。
