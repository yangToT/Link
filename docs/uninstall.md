...
双击安装目录的 `Uninstall.exe`，选择是否清除设备身份。默认选择“否”，保留身份与配置；选择“是”会清除身份，下次需要重新加入。两种方式都会断开网络、恢复 Link 网络设置并移除程序及专属组件。可以先用只读计划查看范围：
...
脚本不带 `-RemoveIdentity` 时保留身份与配置。从 alpha.10 起，执行程序也提供保留选项。升级应使用客户端更新功能，无需先卸载。
